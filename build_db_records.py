...
def main(_):
    ...
    for row in rows[1:]:
        ...
        #print row
        #SOME DATASETS HAVE COLONS IN FILENAMES :/
        ...
        (imagePath, label, startY, startX, endY, endX, occluded,j2,j3) = row
        ...
        p = os.path.sep.join([config.BASE_PATH, "frame/"+imagePath])
        b = D.get(p,[])
        # build a tuple consisting of the label and bounding box,
        # then update the list and store it in the dicti
        if(occluded is '0'):
            print "found good frame "+imagePath
            b.append((label, (startX, startY, endX, endY)))
            D[p]=b
        else:
            print "found OCCLUDED frame "+imagePath
    ...
    for (dType, keys, outputPath) in datasets:
        ...
        for k in keys:
            ...
            for (label, (startX, startY, endX, endY)) in D[k]:
                ...
                xMin = constrain(xMin,0,1)
                xMax = constrain(xMax,0,1)
                yMin = constrain(yMin,0,1)
                yMax = constrain(yMax,0,1)

                print xMin,xMax,yMin,yMax

                #optinal inspect step:
                if inspect:
                    image = cv2.imread(k)
                    startX = int(xMin*w)
                    startY = int(yMin*h)
                    ...
                    cv2.rectangle(image,(startY,startX),(endY,endX),(0,255,0),2)
                ...
                # increment the total num of examples
                total+=1
